...
```bash
# 1) 装 / 升级到 PyPI 上的最新版(应该拿到 1.0.1)
pip install --upgrade aisurface
pip show aisurface                  # Version: 1.0.1

# 2) CLI 三个动词都出来
aisurface --help                    # 看到 {audit, fix, verify}

# 3) 跑内置 fixture — 期望 health 16/100,4 项子分,12 项检查全显示
aisurface audit evals/fixtures/bad-readme-python-lib --no-color

# 4) 跑内置"完美" fixture — 期望 90+ 分,作为上限 sanity check
...
# 6) 拿自己的项目跑(预期和 fixture 不一样的数字,这个 OK)
aisurface audit /path/to/your/repo --no-color

# 7) (可选) verify 真实 AI 引用率 — 需要 PERPLEXITY_API_KEY
export PERPLEXITY_API_KEY=pplx-...
aisurface verify /path/to/your/repo        # 第一次跑 = 建基线,存到 ~/.aisurface/baselines/
```

跑通 1-5 步就基本确认 v1.0.1 包装是对的(没 broken import、没缺文件、CLI 三个 verb 都活)。第 6 步是真实自测 — 在你自己一个真实项目上跑。第 7 步才需要 API key,跳过也行。
